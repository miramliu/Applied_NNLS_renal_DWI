...
function RunAndSave_voxelwise_fourpeaks(PatientNum, ROItype,SignalInput)
    ...
    for voxelj = 1:size(SignalInput,2)
        ...
        %% run fit, and check lambda value in line 71 in RunNNLS_ML_fourpeaks. - HERE IT'S WRITTEN IN FOR EASE
        [~, rsq, ~, ~, resultsPeaks] = RunNNLS_ML_fourpeaks(currcurve,0.1); %best results so far regarding Mann-Whitney U & AUC
...
% for standard  sorted ones, with 4 peaks
    ...
    Existing_Data = readcell(ExcelFileName,'Range','A:B','Sheet','Sortedfourpeaks_lambda_01'); %read only identifying info that already exists
    ...
    if sum(idx)==0
        ...
        writecell(Export_Cell,ExcelFileName,'WriteMode','append','Sheet','Sortedfourpeaks_lambda_01')
    end
%}
