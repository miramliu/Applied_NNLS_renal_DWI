...
function RA_DiffusionSpec_Voxelwise_spectrumexport(varargin)
...
function RunAndSave_voxelwise_spectrum(PatientNum, ROItype,SignalInput)
    ...
    for voxelj = 1:size(SignalInput,2)
        ...
        [DiffusionSpec, rsq, ~, ~, resultsPeaks] = RunNNLS_ML(currcurve); %best results so far regarding Mann-Whitney U & AUC
        
        
        AverageSpect = zeros(size(DiffusionSpec));
        if rsq>0.7 
            if resultsPeaks(1)<1000 %it's set to 10000 if no peaks found, see line 32 of NNLS_result_mod
                %plot(DiffusionSpec)
                %pause(1)
                AverageSpec = AverageSpect + DiffusionSpec;
                count = count + 1;
            end
        end
    end
