...
function RA_DiffusionSpec_Voxelwise_threepeaks(varargin)
    ...
    if contains(varargin{1}, 'V') %then is volunteer, with two kidneys
        ...
        RunAndSave_voxelwise_threepeaks([PatientNum '_RK'],'C',SignalInput)
        ...
        ab = 12;
        SignalInput = ReadPatientDWIData_voxelwise(PatientNum, cortreg, ab);
        RunAndSave_voxelwise_threepeaks([PatientNum '_LK'],'C',SignalInput)
        %}
    else %is allograft study, so only one allograft
        if nargin == 1 || nargin == 2 && varargin{2} <10 
            if nargin == 2
                if varargin{2} ==1 %assumes it's ISMMS site 01
                    PatientNum = ['RA_01_'  varargin{1}];
                else
            ...
            RunAndSave_voxelwise_threepeaks(PatientNum,'C',SignalInput)
...
function RunAndSave_voxelwise_threepeaks(PatientNum, ROItype,SignalInput)
...
% for standard not sorted ones, with 3 peaks
%{
    %Patient ID	ROI Type	mean	stdev	median	skew	kurtosis	size n

    Identifying_Info = {[PatientNum], [PatientNum '_' ROItype]};
    Existing_Data = readcell(ExcelFileName,'Range','A:B','Sheet','Voxelwise_3peak_JONAS'); %read only identifying info that already exists
    ...
    if sum(idx)==0
        ...
        Export_Cell = [Identifying_Info,dataarray,dataarray_peaknumbers];
        writecell(Export_Cell,ExcelFileName,'WriteMode','append','Sheet','Voxelwise_3peak_JONAS')
    end
%}



    %% for sorted ones with 3 peaks
    %Patient ID	ROI Type	mean	stdev	median	skew	kurtosis	size n
