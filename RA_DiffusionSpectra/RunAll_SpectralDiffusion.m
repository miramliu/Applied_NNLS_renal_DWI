

%% Run all renal allograft IVIM cases

%% Site 1 (mount sinai)

RA_DiffusionSpec_Voxelwise_fourpeaks('P001')
RA_DiffusionSpec_Voxelwise_fourpeaks('P002')
RA_DiffusionSpec_Voxelwise_fourpeaks('P003')


RA_DiffusionSpec_Voxelwise_fourpeaks('P006')
RA_DiffusionSpec_Voxelwise_fourpeaks('P007')
RA_DiffusionSpec_Voxelwise_fourpeaks('P008')
RA_DiffusionSpec_Voxelwise_fourpeaks('P009')
RA_DiffusionSpec_Voxelwise_fourpeaks('P010')
RA_DiffusionSpec_Voxelwise_fourpeaks('P011')
RA_DiffusionSpec_Voxelwise_fourpeaks('P012')


RA_DiffusionSpec_Voxelwise_fourpeaks('P014')


RA_DiffusionSpec_Voxelwise_fourpeaks('P017')
RA_DiffusionSpec_Voxelwise_fourpeaks('P018')
RA_DiffusionSpec_Voxelwise_fourpeaks('P019')

RA_DiffusionSpec_Voxelwise_fourpeaks('P021')

RA_DiffusionSpec_Voxelwise_fourpeaks('P023')


RA_DiffusionSpec_Voxelwise_fourpeaks('P026')
RA_DiffusionSpec_Voxelwise_fourpeaks('P027')
RA_DiffusionSpec_Voxelwise_fourpeaks('P028')
RA_DiffusionSpec_Voxelwise_fourpeaks('P029')
RA_DiffusionSpec_Voxelwise_fourpeaks('P030')
RA_DiffusionSpec_Voxelwise_fourpeaks('P031')
RA_DiffusionSpec_Voxelwise_fourpeaks('P032')
RA_DiffusionSpec_Voxelwise_fourpeaks('P033')
RA_DiffusionSpec_Voxelwise_fourpeaks('P034')
RA_DiffusionSpec_Voxelwise_fourpeaks('P035')

RA_DiffusionSpec_Voxelwise_fourpeaks('P037')
RA_DiffusionSpec_Voxelwise_fourpeaks('P038')
RA_DiffusionSpec_Voxelwise_fourpeaks('P039')
RA_DiffusionSpec_Voxelwise_fourpeaks('P040')
RA_DiffusionSpec_Voxelwise_fourpeaks('P041')
RA_DiffusionSpec_Voxelwise_fourpeaks('P042')
RA_DiffusionSpec_Voxelwise_fourpeaks('P043')
RA_DiffusionSpec_Voxelwise_fourpeaks('P044')
RA_DiffusionSpec_Voxelwise_fourpeaks('P045')



RA_DiffusionSpec_Voxelwise_fourpeaks('P049')
RA_DiffusionSpec_Voxelwise_fourpeaks('P050')
RA_DiffusionSpec_Voxelwise_fourpeaks('P051')
RA_DiffusionSpec_Voxelwise_fourpeaks('P052')

RA_DiffusionSpec_Voxelwise_fourpeaks('P054')
RA_DiffusionSpec_Voxelwise_fourpeaks('P055')
RA_DiffusionSpec_Voxelwise_fourpeaks('P056')
RA_DiffusionSpec_Voxelwise_fourpeaks('P057')
RA_DiffusionSpec_Voxelwise_fourpeaks('P058')
RA_DiffusionSpec_Voxelwise_fourpeaks('P059')
RA_DiffusionSpec_Voxelwise_fourpeaks('P060')




% to be processed when ROIs and results are available. Scans ready for
% processing

RA_DiffusionSpec_Voxelwise_fourpeaks('P061')
RA_DiffusionSpec_Voxelwise_fourpeaks('P062')
RA_DiffusionSpec_Voxelwise_fourpeaks('P063')
RA_DiffusionSpec_Voxelwise_fourpeaks('P064')
RA_DiffusionSpec_Voxelwise_fourpeaks('P065')
RA_DiffusionSpec_Voxelwise_fourpeaks('P066')
RA_DiffusionSpec_Voxelwise_fourpeaks('P067')
RA_DiffusionSpec_Voxelwise_fourpeaks('P068')
RA_DiffusionSpec_Voxelwise_fourpeaks('P069')
RA_DiffusionSpec_Voxelwise_fourpeaks('P070')
RA_DiffusionSpec_Voxelwise_fourpeaks('P071')
RA_DiffusionSpec_Voxelwise_fourpeaks('P073')
RA_DiffusionSpec_Voxelwise_fourpeaks('P074')
RA_DiffusionSpec_Voxelwise_fourpeaks('P075')
RA_DiffusionSpec_Voxelwise_fourpeaks('P076')
RA_DiffusionSpec_Voxelwise_fourpeaks('P077')

RA_DiffusionSpec_Voxelwise_fourpeaks('P079')
RA_DiffusionSpec_Voxelwise_fourpeaks('P080')
RA_DiffusionSpec_Voxelwise_fourpeaks('P081')
RA_DiffusionSpec_Voxelwise_fourpeaks('P082')
RA_DiffusionSpec_Voxelwise_fourpeaks('P083')
RA_DiffusionSpec_Voxelwise_fourpeaks('P084')
RA_DiffusionSpec_Voxelwise_fourpeaks('P085')
RA_DiffusionSpec_Voxelwise_fourpeaks('P086')
RA_DiffusionSpec_Voxelwise_fourpeaks('P087')
RA_DiffusionSpec_Voxelwise_fourpeaks('P088')
RA_DiffusionSpec_Voxelwise_fourpeaks('P089')
RA_DiffusionSpec_Voxelwise_fourpeaks('P090')
%}
RA_DiffusionSpec_Voxelwise_fourpeaks('P091')
RA_DiffusionSpec_Voxelwise_fourpeaks('P092')
RA_DiffusionSpec_Voxelwise_fourpeaks('P093')

RA_DiffusionSpec_Voxelwise_fourpeaks('P095')
RA_DiffusionSpec_Voxelwise_fourpeaks('P096')
RA_DiffusionSpec_Voxelwise_fourpeaks('P097')
RA_DiffusionSpec_Voxelwise_fourpeaks('P098')
RA_DiffusionSpec_Voxelwise_fourpeaks('P099')
RA_DiffusionSpec_Voxelwise_fourpeaks('P102')
RA_DiffusionSpec_Voxelwise_fourpeaks('P103')




% final round 
RA_DiffusionSpec_Voxelwise_fourpeaks('P104')
RA_DiffusionSpec_Voxelwise_fourpeaks('P105')
RA_DiffusionSpec_Voxelwise_fourpeaks('P106')
RA_DiffusionSpec_Voxelwise_fourpeaks('P107')
RA_DiffusionSpec_Voxelwise_fourpeaks('P108')
RA_DiffusionSpec_Voxelwise_fourpeaks('P109')
RA_DiffusionSpec_Voxelwise_fourpeaks('P110')
RA_DiffusionSpec_Voxelwise_fourpeaks('P111')
RA_DiffusionSpec_Voxelwise_fourpeaks('P112')
RA_DiffusionSpec_Voxelwise_fourpeaks('P113')
RA_DiffusionSpec_Voxelwise_fourpeaks('P114')








%% site 2 (cornell)
%for center 2, change PatientNumb to RA_02 rather than RA_01 in
%RA_DiffusionSpec_Voxelwise_fourpeaks

RA_DiffusionSpec_Voxelwise_fourpeaks('P003',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P004',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P005',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P006',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P007',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P008',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P009',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P010',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P011',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P012',2)


RA_DiffusionSpec_Voxelwise_fourpeaks('P015',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P016',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P017',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P019',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P020',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P021',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P022',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P024',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P026',2)


RA_DiffusionSpec_Voxelwise_fourpeaks('P029',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P032',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P033',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P034',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P035',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P037',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P038',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P040',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P041',2)
%}


RA_DiffusionSpec_Voxelwise_fourpeaks('P041',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P042',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P043',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P045',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P046',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P047',2)

RA_DiffusionSpec_Voxelwise_fourpeaks('P049',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P053',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P054',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P055',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P056',2)







% final set
RA_DiffusionSpec_Voxelwise_fourpeaks('P057',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P060',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P061',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P062',2)
RA_DiffusionSpec_Voxelwise_fourpeaks('P064',2)




%% healthy volunteers (thank you!!)
%must change 
%{
RA_DiffusionSpec_Voxelwise_fourpeaks('V001') %change line 16/17 to RA_01
RA_DiffusionSpec_Voxelwise_fourpeaks('V002') %change line 16/17 to RA_01
RA_DiffusionSpec_Voxelwise_fourpeaks('V003') %change line 16/17 to RA_01
RA_DiffusionSpec_Voxelwise_fourpeaks('V004') %change line 16/17 to RA_01
%RA_DiffusionSpec_Voxelwise_fourpeaks('V005') %change line 16/17 to RA_01

RA_DiffusionSpec_Voxelwise_fourpeaks('V001',2) %change line 16/17 to RA_02




%}