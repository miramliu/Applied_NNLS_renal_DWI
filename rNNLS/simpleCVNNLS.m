...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Thomas Gladytz 2024.07.17 Editted to speed up regularization for Mira Liu
% use regularization parameter lambda as the 3rd input argument, which should be on the order of number of b=values/SNR 
% Now outputs unregularized NNLS results as well, and does not regularize the edges of the spectrum no longer assuming there is no diffusion
...
function [ xOut, ChiUsed, resid, xUnreg] = simpleCVNNLS(A,Decay,Lambda)
...
%Curvature regularization; standard L2-norm regularition acts stronger on intense peaks than on weak peaks.
H([1 end])=0.5; % the ends of the spectrum shall not be downregularized specifically.
...
% chi2_min
[ xUnreg, resnormMin] = lsqnonneg(  A, Decay  ); 
% chi2_smooth
y_recon = A * xOut;
resid = Decay - y_recon;
